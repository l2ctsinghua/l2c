...
(*  Agreement.                                                         *)
(*                                                                     *)
(*  we modify some variable names to make the proof work               *)
(*                                                                     *)
(* *********************************************************************)
...
with eval_funcall: mem -> fundef -> list val -> trace -> mem -> val -> Prop :=
  | eval_funcall_internal: forall le m f vargs t e le1 m1 m2 m3 out vres,
      list_norepet (var_names f.(fn_vars)) ->
      list_norepet (var_names f.(fn_params)) ->
      list_disjoint (var_names f.(fn_params)) (var_names f.(fn_temps)) ->
      alloc_variables empty_env m f.(fn_vars) e m1 ->
      bind_parameter_temps f.(fn_params) vargs (create_undef_temps f.(fn_temps)) = Some le ->
      exec_stmt e le m1 f.(fn_body) t le1 m2 out ->
...
with evalinf_funcall: mem -> fundef -> list val -> traceinf -> Prop :=
  | evalinf_funcall_internal: forall m f vargs t e le m1,
      list_norepet (var_names f.(fn_vars)) ->
      list_norepet (var_names f.(fn_params)) ->
      list_disjoint (var_names f.(fn_params)) (var_names f.(fn_temps)) ->
      alloc_variables empty_env m f.(fn_vars) e m1 ->
      bind_parameter_temps f.(fn_params) vargs (create_undef_temps f.(fn_temps)) = Some le ->
      execinf_stmt e le m1 f.(fn_body) t ->
...
(* call internal *)
  ...
  eapply star_right. eexact A1.
   inv B1; simpl in *; try contradiction.
  (* Out_normal *)
