...
Fixpoint remove_all_leaves (n:nat)(t:bin){struct t} : bin :=
  match t with
    leaf n => leaf n
  | node (leaf n1) t2 =>
    if beq_nat n n1 then remove_all_leaves n t2 else t
  | _ => t
  end.
...
intros Heq A r l def; rewrite (beq_nat_eq x n); auto.
Qed.
...
simpl; auto.
intros A r l def.
generalize (refl_equal (beq_nat x n)); pattern (beq_nat x n) at -1;
 case (beq_nat x n); simpl; auto.
intros Heq_nateq.
assert (Heq_xn : x=n).
apply beq_nat_eq; rewrite Heq_nateq;unfold Is_true;auto.
rewrite Heq_xn.
